...
Parser& Parser::add_flag_parameter(const std::string& flag_name, string_view description) {
  ...
  // provided
  this->parameters[flag_name] =
      ProgramParameter{nullptr, std::any(), std::string(description), true, false, ""};
  return *this;
}
...
/**
 * @brief Given a raw program argument, extract a `Token` from it based on what
 * symbols the argument contains
 *
 * @param argument The raw program argument passed by the end user
 * @return Token A `PositionalArgument` if the argument did not have a leading
 * `--`, a `Flag` if the argument had a leading `--` but did not contain an
 * equal sign, or a `ParameterWithValue` if the argument both had a leading `--`
 * and an equal sign
 */
Token extract_token_from_program_argument(string_view argument) {
  if (argument.starts_with("--")) {
    ...
    if (contains_equal_sign) {
      ...
      return ParameterWithValue{string(parameter_name), string(argument)};
    } else {
...
expected<optional<string>, string> Parser::handle_parameter_token_with_value(
  ...
  if (is_flag_registered(parameter_name)) {
    ...
    return unexpected(std::format("Found unknown parameter: --{}", parameter_name));
  }
...
expected<optional<string>, string> Parser::handle_flag_token(const string& flag_name) {
  if (is_parameter_registered(flag_name)) {
    ...
    return unexpected(std::format("Found unknown flag: --{}", flag_name));
  }
...
expected<vector<string>, vector<string>> Parser::parse_arguments(
  ...
  if (!result) {
    ...
    errors.insert(errors.end(), std::make_move_iterator(missing_errors.begin()),
...
expected<void, string> Parser::attempt_parse(const string& parameter_name, string_view input) {
  ...
  if (!result) {
    ...
    return std::unexpected(std::format("For parameter --{}: {}", parameter_name, error_message));
  }
...
bool Parser::is_flag_set(const string& flag_name) {
  if (is_parameter_registered(flag_name)) {
    ...
    std::cerr << "Developer error: This parser is not configured with a flag named \"" << flag_name
              << '"' << std::endl;
    std::exit(1);
  }
